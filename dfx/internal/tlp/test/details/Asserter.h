...
static_assert(__value(__valid_type(T)), "Assert Failed: expect "#T" be valid, but be invalid!")

#define ASSERT_INVALID(T)               \
static_assert(!(__value(__valid_type(T))), "Assert Failed: expect "#T" be invalid, but be valid!")
